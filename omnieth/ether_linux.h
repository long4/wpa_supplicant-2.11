...
#include <soc/ob01/compile_marco.h>
...
#if defined(OB_NET_PAGE_POOL_H_PRESENT)
...
#include <soc/ob01/ptp-notifier.h>
...
#ifdef ETHER_GRO
/* GRO packets purge threshold in msec */
#define GRO_AGE_THRESHOLD		500
#define GRO_PURGE_TIMER_THRESHOLD	5000
#define GRO_RX_RUNNING		OSI_BIT(0)
#define GRO_PURGE_TIMER_RUNNING	OSI_BIT(1)
#endif
...
struct ether_xtra_stat_counters {
	...
	u64 re_alloc_rxbuf_failed[OSI_MGBE_MAX_NUM_CHANS];
	/** TX per channel interrupt count */
	u64 tx_normal_irq_n[OSI_MGBE_MAX_NUM_CHANS];
	/** TX per channel SW timer callback count */
	u64 tx_usecs_swtimer_n[OSI_MGBE_MAX_NUM_CHANS];
	/** RX per channel interrupt count */
	u64 rx_normal_irq_n[OSI_MGBE_MAX_NUM_CHANS];
	/** link connect count */
	u64 link_connect_count;
	/** link disconnect count */
	u64 link_disconnect_count;
};
...
struct ether_priv_data {
	...
	/** MACsec debugfs directory */
	struct dentry *macsec_debugfs_dir;
#ifdef CONFIG_MACSEC
	/** MACsec standard offload context */
	void *macsec_ctx;
#endif /* CONFIG_MACSEC */
	...
	/** Rx processing state for GRO */
	atomic_t rx_state;
	/** Purge timer state for GRO */
	atomic_t timer_state;
	/** GRO packet age threshold in milseconds */
	u32 pkt_age_msec;
	/** GRO purge timer interval */
	u32 nvgro_timer_intrvl;
	/** GRO packet dropped count */
	u64 nvgro_dropped;
#endif
 ...
 * @param[in] pdata: Ethernet driver private data
 *
 * @note  ob01 ethss sysfs group need to be registered during probe.
 */
...
static inline u64 update_stats_counter(u64 last_value, u64 incr)
{
	u64 temp = last_value + incr;
...
#ifdef ETHER_GRO
void ether_gro_purge_timer(struct timer_list *t);
#endif /* ETHER_GRO */
#ifdef BW_TEST
